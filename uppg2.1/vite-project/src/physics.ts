...
export function handleCircleCollision(a: CircleBody, b: CircleBody): void {
  ...
  const aMomentum = (aDotNormal * (a.radius - b.radius) + 1.85 * b.radius * bDotNormal) / (a.radius + b.radius);
  const bMomentum = (bDotNormal * (b.radius - a.radius) + 1.85 * a.radius * aDotNormal) / (a.radius + b.radius);
